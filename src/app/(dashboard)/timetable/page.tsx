...
export default function TimetablePage() {
  ...
  const getSubjectColor = (subject: string) => {
    return (
      SUBJECT_COLORS[subject] || "bg-muted text-foreground border-border"
    );
  };
        ...
        <div className="flex gap-2">
          <Select value={selectedClass} onValueChange={setSelectedClass}>
            <SelectTrigger className="w-40">
              <SelectValue placeholder={`Select ${classLabel.toLowerCase()}`} />
            </SelectTrigger>
            <SelectContent>
              {classes.map((c) => (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={`Select or type ${classLabel.toLowerCase()}`}
                      />
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-muted/50">
                  <th className="border p-2 text-left text-sm font-semibold text-muted-foreground w-20">
                    Period
                  </th>
                  <th className="border p-2 text-left text-xs text-muted-foreground w-16">
                    Time
                  </th>
                  {DAYS.map((d) => (
                    <th
                      key={d}
                      className="border p-2 text-center text-sm font-semibold text-muted-foreground"
                    >
                      {d.slice(0, 3)}
                    </th>
                  ))}
                      ...
                      return (
                        <td key={day} className="border p-1 min-w-[100px]">
                          {period ? (
                            ...
                            <div
                              className={`rounded-lg p-2 text-center text-xs text-gray-300 ${canAdd ? "cursor-pointer hover:bg-muted hover:text-muted-foreground" : ""}`}
                              onClick={() => {
                                if (canAdd) {
                                  setForm({
                                    ...form,
