...
function renderMarkdown(text: string): string {
  let html = text
    ...
    .replace(
      /`([^`]+)`/g,
      '<code class="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-sm font-mono text-primary">$1</code>',
    )
    // Bold
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={cn(
            "fixed z-50 flex flex-col bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl shadow-slate-900/20 transition-all duration-300",
            isExpanded
              ? "inset-4 rounded-2xl"
        ...
        >
                ...
                <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
                  {SUGGESTED_PROMPTS.map((prompt) => (
                    <button
                      ...
                      className="text-left p-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-violet-300 dark:hover:border-violet-600 hover:shadow-md transition-all duration-200 group"
                    >
          ...
          <div className="px-4 pb-4 pt-2 border-t border-slate-200 dark:border-slate-800">
            <div className="flex items-end gap-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 focus-within:border-violet-400 dark:focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-500/20 transition-all duration-200 p-1">
              <textarea
                ref={inputRef}
                value={input}
